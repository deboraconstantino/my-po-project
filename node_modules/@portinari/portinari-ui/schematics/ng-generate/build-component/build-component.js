...
function buildComponent(options) {
    return (host) => {
        ...
        const project = project_2.getProjectFromWorkspace(workspace, options.project);
...
function addImportToModule(options) {
    return (tree) => {
        ...
        const componentModulePath = core_1.normalize(`/${options.path}/`
...
function addDeclarationToModule(options) {
    return (tree) => {
        ...
        const modulePath = options.module;
        const componentPath = core_1.normalize(`/${options.path}/`
            + core_1.strings.dasherize(options.name) + '/'
            + core_1.strings.dasherize(options.name)
            + '.component');
